...
constexpr uint32_t debug_stage = get_compile_time_arg_val(3);
...
constexpr auto cb_packed_partials_output = tt::CBIndex::c_29;
...
void accumulate_dw2_for_row() {
    constexpr uint32_t reg_x = 0U;
    constexpr uint32_t reg_dout = 1U;
    constexpr uint32_t reg_inv = 2U;
    constexpr uint32_t reg_sum = 3U;
    ...
    tile_regs_commit();
    pack_and_push(reg_sum, cb_sum_x2);
    reduce_sum_to_scalar(cb_sum_x2, cb_sum_scale_1);
}

void accumulate_dw1_for_row() {
    constexpr uint32_t reg_x = 0U;
    constexpr uint32_t reg_dout = 1U;
    constexpr uint32_t reg_inv = 2U;
    constexpr uint32_t reg_sum = 3U;
    ...
    tile_regs_commit();
    pack_and_push(reg_sum, cb_sum_x4);
    reduce_sum_to_scalar(cb_sum_x4, cb_sum_scale_2);
}

void accumulate_dw0_for_row() {
    constexpr uint32_t reg_x = 0U;
    constexpr uint32_t reg_dout = 1U;
    constexpr uint32_t reg_inv = 2U;
    constexpr uint32_t reg_sum = 3U;
    ...
    tile_regs_commit();
    pack_and_push(reg_sum, cb_sum_x6);
    reduce_sum_to_scalar(cb_sum_x6, cb_sum_scale_3);
}

void accumulate_db_for_row() {
    ...
    tile_regs_commit();
    pack_and_push(reg_sum, cb_sum_x2);
    reduce_sum_to_scalar(cb_sum_x2, cb_inv_rms_x);
}

void drain_db_pass_for_row() {
    for (uint32_t col = 0; col < num_inner; col += block_size) {
        cb_wait_front(cb_dL_dout_pass_4, block_size);
        cb_pop_front(cb_dL_dout_pass_4, block_size);
    }
...
void emit_packed_partials_for_row() {
    binary_op_init_common(cb_input_pass_1, cb_input_pass_1, cb_packed_partials_output);

    if constexpr (debug_stage == 1U) {
        cb_wait_front(cb_sum_scale_1, onetile);
        cb_wait_front(cb_sum_scale_2, onetile);
        cb_wait_front(cb_sum_scale_3, onetile);
        cb_wait_front(cb_inv_rms_x, onetile);
        cb_wait_front(cb_inv_rms_x2, onetile);
        cb_wait_front(cb_inv_rms_x3, onetile);
        ...
        copy_tile(cb_inv_rms_x, 0, reg0);
        copy_tile_init(cb_inv_rms_x2);
        copy_tile(cb_inv_rms_x2, 0, reg1);
        copy_tile_init(cb_inv_rms_x3);
        copy_tile(cb_inv_rms_x3, 0, reg2);
        copy_tile_init(cb_one);
        copy_tile(cb_one, 0, reg3);
        sub_binary_tile_init();
        sub_binary_tile(reg3, reg3, reg3);
        tile_regs_commit();

        cb_reserve_back(cb_packed_partials_output, block_size);
        tile_regs_wait();
        pack_reconfig_data_format(cb_packed_partials_output);
        pack_tile(reg0, cb_packed_partials_output);
        pack_tile(reg1, cb_packed_partials_output);
        pack_tile(reg2, cb_packed_partials_output);
        pack_tile(reg3, cb_packed_partials_output);
        tile_regs_release();
        cb_push_back(cb_packed_partials_output, block_size);

        cb_pop_front(cb_sum_scale_1, onetile);
        cb_pop_front(cb_sum_scale_2, onetile);
        cb_pop_front(cb_sum_scale_3, onetile);
    ...
    constexpr uint32_t reg2 = 2U;
    constexpr uint32_t reg3 = 3U;
    copy_tile_init(cb_sum_scale_3);
    copy_tile(cb_sum_scale_3, 0, reg0);
    copy_tile_init(cb_sum_scale_2);
    copy_tile(cb_sum_scale_2, 0, reg1);
    copy_tile_init(cb_sum_scale_1);
    copy_tile(cb_sum_scale_1, 0, reg2);
    copy_tile_init(cb_inv_rms_x);
    copy_tile(cb_inv_rms_x, 0, reg3);
    tile_regs_commit();
    cb_reserve_back(cb_packed_partials_output, block_size);
    tile_regs_wait();
    pack_reconfig_data_format(cb_packed_partials_output);
...
void kernel_main() {
    ...
    for (uint32_t row = 0; row < num_rows_per_core; ++row) {
        ...
        accumulate_dw2_for_row();
        accumulate_dw1_for_row();
        accumulate_dw0_for_row();
        emit_output_for_row();
        if constexpr (debug_stage == 1U) {
            drain_db_pass_for_row();
        } else {
            accumulate_db_for_row();
        }
