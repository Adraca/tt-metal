...
public:
    ...
    // view() returns the original (unpadded) dimensions, so shapes with different ranks hash
    // differently. Using value_ directly would cause collisions because ShapeBase::init() pads
    // to min-4D with leading 1s (e.g., [128,128] and [1,128,128] both store {1,1,128,128}).
